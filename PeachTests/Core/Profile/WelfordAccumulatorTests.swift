...
struct WelfordAccumulatorTests {
    ...
    // MARK: - Accumulator with Double values

    @Test("empty accumulator has zero count and nil accessors")
    func emptyAccumulator() async {
        let acc = WelfordAccumulator()
        #expect(acc.count == 0)
        #expect(acc.mean == 0.0)
        #expect(acc.sampleStdDev == nil)
        #expect(acc.populationStdDev == nil)
    }

    @Test("single update sets mean")
    func singleUpdate() async {
        ...
        #expect(acc.sampleStdDev == nil) // need >= 2 for sample stddev
    }
    ...
    @Test("sample stddev returns correct value")
    func sampleStdDev() async throws {
        var acc = WelfordAccumulator()
        acc.update(10.0)
        acc.update(20.0)
        acc.update(30.0)
        ...
        let stddev = try #require(acc.sampleStdDev)
        #expect(abs(stddev - 10.0) < 0.01)
    }
