...
struct PitchMatchingProfileTests {
    ...
    func matchingMean() async {
        let profile = PerceptualProfile()
        PitchMatchingProfileAdapter(profile: profile).pitchMatchingCompleted(CompletedPitchMatchingTrial(referenceNote: 60, targetNote: 60, initialCentOffset: 50.0, interval: .prime, userCentError: 10.0, tuningSystem: .equalTemperament))
        PitchMatchingProfileAdapter(profile: profile).pitchMatchingCompleted(CompletedPitchMatchingTrial(referenceNote: 60, targetNote: 60, initialCentOffset: 50.0, interval: .prime, userCentError: -20.0, tuningSystem: .equalTemperament))
    ...
    func observerUpdatesStats() async {
        let profile = PerceptualProfile()
        let result = CompletedPitchMatchingTrial(referenceNote: 60, targetNote: 60, initialCentOffset: 50.0, interval: .prime, userCentError: 15.0, tuningSystem: .equalTemperament)
