...
struct AdaptiveTimingOffsetDetectionStrategyTests {
    ...
    func coldStartReturnsMaxOffset() async {
        let strategy = AdaptiveTimingOffsetDetectionStrategy()
        let settings = TimingOffsetDetectionSettings(tempo: TempoBPM(120), maxOffsetPercentage: 20.0)
    ...
    func coldStartPassesTempo() async {
        let strategy = AdaptiveTimingOffsetDetectionStrategy()
        let settings = TimingOffsetDetectionSettings(tempo: TempoBPM(100))
    ...
    func favorsWeakerDirection() async {
        ...
        let settings = TimingOffsetDetectionSettings(tempo: TempoBPM(80))
    ...
    func coldStartWithProfileMean() async {
        ...
        for direction in TimingDirection.allCases {
            let signedOffset = direction == .early
                ? TimingOffset(.zero - .milliseconds(50))
                : offset
            TimingOffsetDetectionProfileAdapter(profile: profile).timingOffsetDetectionCompleted(CompletedTimingOffsetDetectionTrial(
                tempo: TempoBPM(80),
                offset: signedOffset,
    ...
    func convergenceFromMax() async {
        ...
        var trial = strategy.nextTimingOffsetDetectionTrial(
            profile: PerceptualProfile(),
            settings: settings,
            lastResult: nil
        )
        ...
        for _ in 0..<10 {
            ...
            trial = strategy.nextTimingOffsetDetectionTrial(
                profile: PerceptualProfile(),
                settings: settings,
    ...
    private func makeCompleted(
        ...
        return CompletedTimingOffsetDetectionTrial(
            tempo: tempo,
            offset: offset,
            isCorrect: correct
        )
