...
2. **`Rhythm*` vs "Timing" naming split**: *Resolved.* The TOD-specific types were renamed to `TimingOffset`, `TimingDirection`, `TimingOffsetDetection*` (and the discipline ID is now `timingOffsetDetection`). The only `Rhythm*` domain type still in source is `RhythmVelocity`, which both rhythm disciplines use as their per-subdivision dynamic. The previously hypothesised cross-discipline shared types (`RhythmOffset`, `RhythmDirection`, `RhythmProfile`) do not exist in source — Continuous Rhythm Matching also speaks in `TimingOffset` / `TimingDirection`, so the rename was effectively total. The view type `RhythmProfileCardView` is unrelated.
