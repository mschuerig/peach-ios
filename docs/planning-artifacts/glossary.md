...
| **Offset Note** | In *Rhythm Offset Detection* (see § *Naming Matrix*), the single note in the looping rhythmic figure that carries a small temporal offset (early or late) relative to its grid position on a given trial. The user-pickable parameter is the *Offset Note Position* — a 1-based index into the audible (non-rest) notes of the active pattern. Names *which note* the offset is applied to; it is **not** the magnitude of the offset (that is a separate timing-offset value) and shares only the word "offset" with the pitch disciplines' *Cent Offset* — they are not analogues (Cent Offset is a magnitude applied to a single target note; *Offset Note* is a role chosen across multiple candidate notes in a pattern). Code identifier: `offsetNotePosition`. The audible position is named *Note* in user-facing surfaces; engineering layers may speak of *slots* in the pattern data structure, but *slot* is never the user-facing noun. The term explicitly excludes the music-theory notion of *rhythmic displacement* (intentional, grid-aware metric shifts), which TOD does not train. |
...
| **Training Discipline Config** | A `TrainingDisciplineConfig` struct holding per-discipline configuration for progress tracking: display name, unit label, optimal baseline, EWMA half-life, and session gap. One instance per registered discipline. |
...
| **Profile Screen** | The full perceptual profile visualization. Shows the piano keyboard with confidence band overlay, summary statistics with trend, pitch matching accuracy, and one progress timeline card per registered training discipline. |
