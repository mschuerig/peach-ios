...
(Acceptance criteria — see `docs/implementation-artifacts/85-3-audit-and-harden-sequencer-concurrency.md`. Closes PF-011 from `docs/implementation-artifacts/deferred-work.md`. Cluster framing: PF-011 consolidates one production-reachable race (stale `samplePosition` at new-trial start with `maxRepetitions == 1`, surfacing as a premature `.repetitionCapReached` before any audio is heard) and a set of unverified static-shape invariants (`BeatProvider` not `Sendable`, `SoundFontBeatSequencer.currentBeat` mutated from a background Task, `ContinuousRhythmMatchingSession.gapPositionInCurrentBeat` written from a polling Task, and the coordinator's implicit "previous session stopped before next starts" serialization). Two adversarial reviewers flagged the static shape; the strict-concurrency build is clean only because `BeatProvider` is not `Sendable` — strict concurrency isn't catching what it can't see. Task 1 of the story is the audit itself: invoke `/swift-concurrency-expert` (and optionally `/avdlee-swift-concurrency` as a second lens) against the named surfaces; produce a Sendable/actor-isolation map, a per-risk recommendation (document-as-contract / add-test / refactor), and an explicit pick from the catalog's three trial-start-race options (document post-`start()` reset latency / per-trial `samplePosition` baseline / `BeatSequencer.timing` trial-relative accessor). Halt for human review before any code change. Subsequent tasks: tests-first regression for the maxReps=1 race; apply the chosen mitigation; coordinator serialization contract test; apply the audit's Sendable / actor-isolation recommendations. Acceptance per PF-011: trial-start race no longer fires `.repetitionCapReached` before any audio is heard; coordinator pins the cross-discipline serialization invariant by test; the sequencer + session polling paths have an explicit concurrency contract (documented inline or enforced by annotation per the audit). All four pre-commit gates green; strict-concurrency build remains clean. Catalog hygiene on close: PF-011 removed from `deferred-work.md` in the closing commit; any audit-surfaced new findings filed as new `PF-###` entries.)
