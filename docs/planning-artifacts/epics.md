...
**Work order:** 77.1 → 77.2 → 77.3 → 77.4 → 77.5 (77.2 builds on 77.1's contribution protocol; 77.3 and 77.4 are independent of each other and may run in either order after 77.2; 77.5 documents the landed state and runs last).
...
This story runs after 77.2 / 77.3 / 77.4 reach `done` so the documentation describes the landed state.

**Acceptance Criteria:**

1. **Given** `architecture.md` ends at v0.8 and the v0.5 amendment is the most recent description of disciplines **When** epic 77's documentation update lands **Then** a new `## v0.9 Architecture Amendment — Plugin-Style Discipline Contributions` is appended after v0.8, in the existing template (motivation paragraph, `**Supersedes:**` block, `**Implementation stories:**` pointer to Epic 77), with subsections covering the two-protocol Core/App split (`TrainingDiscipline` + `TrainingDisciplineUI`), the feature-directory colocation rule, per-discipline compile-time activation in `DisciplineBootstrap.swift`, feature-local storage for `enabledGapPositions`, the post-77 "Adding a new discipline requires" sequence, an updated project-structure snapshot, and an updated central-files inventory.
2. **Given** the agent audience **When** the v0.9 amendment is read **Then** every claim names files, types, protocol members, and compilation flags by their literal code names (e.g., `TrainingDisciplineUI`, `profileCard: AnyView`, `PEACH_RESEARCH`); supersedes-claims quote enough of the v0.5 wording to be unambiguous; subsections close with `[Source: docs/implementation-artifacts/77-N-...md]` references.
3. **Given** `arc42.md` Section 8.7 (Training Discipline Registry) was written for v0.5 **When** updated **Then** it is rewritten in place to describe the post-77 plugin model in arc42's prose-first style: two-protocol split, shorter "Adding a new discipline" list, per-discipline activation paragraph with pointer to ADR-10, colocation closing paragraph. No file inventories or full protocol surfaces — that detail belongs in `architecture.md`.
4. **Given** per-discipline compile-time activation is a deliberate decision with rejected alternatives **When** documented **Then** `arc42.md` Section 9 gains `### ADR-10: Per-Discipline Compile-Time Activation` after ADR-9, in the existing template (Context / Decision / Status / Consequences with (+)/(–) bullets), citing story 77.1.
5. **Given** the rewrite of Section 8.7 + ADR-10 establishes the post-77 picture **When** other `arc42.md` sections are checked **Then** the document header version and date are bumped, Section 12 Glossary gains entries for `TrainingDisciplineUI`, `DisciplineBootstrap`, and `PEACH_RESEARCH` (with `TrainingDisciplineRegistry`'s definition adjusted for the protocol split), and Section 4's Solution-Strategy table row and Section 11's "Original architecture document partially outdated" row are touched only if their existing wording is now misleading.
6. **Given** this story file (`77-5-...md`) is the canonical home of the full requirement text **When** `epics.md` is updated **Then** Story 77.5's epic-level entry summarizes the ACs in one sentence each rather than duplicating the story file's full text, and the work-order paragraph is updated to `77.1 → 77.2 → 77.3 → 77.4 → 77.5`.
7. **Given** the documentation updates land together **When** the touched sections are read in sequence **Then** file paths, type names, and protocol members agree across `architecture.md` v0.9, `arc42.md` Section 8.7 + ADR-10, and `epics.md`; pre-77 amendments (v0.5–v0.8) keep their historical text; touched sections contain no legacy claims (e.g., no "Add a `TrainingDiscipline` enum case", no `if activeCategories.contains(.rhythm)` example).
8. **Given** the changes are documentation-only **When** `bin/test.sh && bin/test.sh -p mac` runs **Then** it is green and `bin/build.sh && bin/build.sh -p mac` produces no new warnings; if any code change appears necessary, the story is mis-scoped and the discrepancy is flagged rather than silently fixed.
