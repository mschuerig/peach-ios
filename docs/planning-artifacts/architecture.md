...
**The architecture is now plugin-style.** A discipline is a self-contained plugin under `Peach/Training/<Feature>/`: one directory holds its session, screen, persistence payload, CSV history, observer protocol, adapters, lifecycle contribution, and help bodies. Central screens iterate `TrainingDisciplineRegistry.shared.allUI` rather than naming disciplines individually; persistence collapses to a single SwiftData envelope; CSV migration derives from per-discipline `CSVHistory` declarations; per-discipline-private settings stay behind feature-local ports. New disciplines are additive — existing files are not modified. The remainder of this amendment details the mechanism.
...
### v0.10 Addendum — Cleanup in async continuations (added 2026-06-06)

Story 85.1's chain pattern is correct for its actual responsibility: serializing intentional stop sequences against subsequent plays. The merge was followed by a deterministic "first reference note silent on quick discipline switch" symptom that proved harder to pin down. Two compounding faults were eventually found that the chain pattern alone cannot prevent — both are about *cleanup paths* rather than the chain itself:

1. **Synchronous-commit-to-chain only enforces order on synchronous code paths.** A cleanup that runs in an async continuation (e.g. the `catch` block of a cancelled `Task.sleep` in `NotePlayer+TimedPlay.swift`) cannot register its chain entry "before" siblings that have already captured the chain tail — the continuation's `MainActor` turn happens at whatever moment the scheduler dispatches it. The rule that follows: **do not let async cleanup paths register chain entries that are already redundant with synchronous session-level stops.** In this codebase, every cancellation propagates through `pause()`/`stop()`, which both issue `scheduleStopAll()` synchronously; the cancelled-trial cleanup is therefore redundant by construction. The fix is to remove it on the `CancellationError` branch, not to chain it.
2. **Don't summon machinery you don't need.** `SoundFontEngine.clearSchedule()` exists to flush the rhythm sequencer's scheduled-event queue and silence stuck notes from the cleared schedule via a deferred render-thread CC#123 dispatch. Pitch sessions have no scheduled events to flush; calling `clearSchedule()` from `SoundFontPlayer.scheduleStopAll()` engaged the render-thread CC#123 as a *second* deferred silencer that could fire after a subsequent `play()`'s noteOn. The fix is to not call `clearSchedule()` from pitch's stop path. The synchronous `sampler.sendController(123)` in `stopNotes()` already silences the channel in MainActor source order, which is what pitch needs.

Both rules generalise: **one definitive cleanup beats two redundant ones competing on chain ordering**, and **a layer should invoke only the primitives it actually requires**. The chain primitive's contract is unchanged; the addendum constrains who is allowed to register on it.
