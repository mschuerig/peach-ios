...
- **Training disciplines (build-gated)** — configurations without `PEACH_RESEARCH` (`Debug`, `Release`) register the pitch disciplines only: unison pitch comparison, interval pitch comparison, unison pitch matching, interval pitch matching. Configurations with `PEACH_RESEARCH` (`Debug (Research)`, `Release (Research)`) additionally register timing offset detection and continuous rhythm matching. Each has independent progress tracking via `TrainingDisciplineConfig`. `TrainingDisciplineRegistry` is bootstrapped in `PeachApp.init()` from `DisciplineBootstrap.allDisciplines`; only that file references the build flag. `ProgressTimeline` and `PerceptualProfile` iterate `registry.all`, so they automatically scale with the registered set
