...
- **Training disciplines (build-gated)** — Release/App Store ships four pitch disciplines: unison pitch comparison, interval pitch comparison, unison pitch matching, interval pitch matching. Debug and `PEACH_RESEARCH` builds additionally include timing offset detection and continuous rhythm matching. Each has independent progress tracking via `TrainingDisciplineConfig`. `TrainingDisciplineRegistry` is bootstrapped in `PeachApp.init()` from `DisciplineBootstrap.allDisciplines`; only that file references the build flag. `ProgressTimeline` and `PerceptualProfile` iterate `registry.all`, so they automatically scale with the registered set
