...
- **Two-world architecture** — logical world (`MIDINote`, `DetunedMIDINote`, `Interval`, `Cents` in `Core/Music/`) and physical world (`Frequency` in `Core/Music/`), bridged by `TuningSystem`. Two forward bridges: **absolute** `frequency(for:referencePitch:)` for a single note (A-rooted; Settings previews, reference tones) and **reference-relative** `frequency(for:detunedBy:from:)` for an interval target derived from the reference frequency — interval-trial playback MUST use the reference-relative bridge (the absolute one would tie the JI in-tune point to an inaudible A-root; Story 87.1). Trials carry their `DirectedInterval`; frequency derivation never re-derives it from MIDI distance. Inverse (Hz → MIDI note + cents) is `SoundFontPlayer.decompose(frequency:)` (internal for testability, used only within the SoundFont layer). All bridge parameters are explicit (no defaults)
