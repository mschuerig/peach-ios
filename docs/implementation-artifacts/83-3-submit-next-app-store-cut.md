...
**Open, not yet dispositioned:** (a) the seeded Compare Timing heatmap renders almost entirely red ("Erratic") because `bin/generate-test-data.py` draws random offsets — honest, but it depicts the user as consistently poor in a shipping App Store asset; fixable by regenerating the seed with tighter offsets and re-capturing `07` on both devices, which is seed-data work inside Task 5's remit rather than a code change. (b) On iPhone only, two tempo-range labels truncate to `160–2…` / `120–1…` in that same card; iPad renders them in full, so it is a width-constrained layout issue. New finding, no catalog entry, not caused by this story.
