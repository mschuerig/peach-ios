...
- [x] **Task 7 — Archive and upload.** — **complete.** Build `1.1.0 (2)` uploaded 2026-08-08 20:56 and processed successfully; App Store Connect shows it under Version 1.1.0 as **Ready to Submit** with **no Missing Compliance warning**, confirming `ITSAppUsesNonExemptEncryption = NO` survived into the archive. Scheme verification (five disciplines in the shipping binary) is done via TestFlight on a physical device — the archive is arm64 and cannot be installed on a simulator, so a simulator build would only prove what the scheme does, not what was uploaded. ORIGINAL: **Precondition: confirm `83-4` is `done` in `sprint-status.yaml` before archiving** — it changes the binary. Destination "Any iOS Device (arm64)", scheme **`Peach (Release)`**, clean build folder, Product → Archive. Run Validate App as a dry run, then Distribute App → "TestFlight & App Store" → Upload. Confirm the build reaches "Ready to Submit" in App Store Connect with no Missing Compliance warning.
