...
- [ ] Task 1: Make `bootstrap(disciplines:)` idempotent (AC: 1)
  - [ ] 1.1 Inspect `Peach/Core/Training/Discipline/TrainingDisciplineRegistry.swift` to see how `_shared` is currently set
  - [ ] 1.2 Add a guard so the second call is a no-op (or document the replace-on-second-call semantics)
  - [ ] 1.3 Add a unit test that calls bootstrap twice and asserts no trap, with the first list winning (or whichever semantics the dev chose)
- [ ] Task 2: Create `Peach/App/PreviewSupport.swift` (AC: 2)
  - [ ] 2.1 Wrap entire file in `#if DEBUG` / `#endif`
  - [ ] 2.2 Define `enum PreviewSupport { static func bootstrapRegistryIfNeeded() }`
- [ ] Task 3: Wire every relevant `#Preview` (AC: 3)
  - [ ] 3.1 `grep -rn "TrainingDisciplineRegistry.shared" Peach/` → list of files
  - [ ] 3.2 In each file with `#Preview` blocks, prepend `PreviewSupport.bootstrapRegistryIfNeeded()` to the preview body
  - [ ] 3.3 Spot-check the preview canvas in Xcode for each touched view
- [ ] Task 4: Build and test both platforms (AC: 5)
  - [ ] 4.1 `bin/test.sh && bin/test.sh -p mac`
  - [ ] 4.2 `bin/build.sh && bin/build.sh -p mac` — zero new warnings
