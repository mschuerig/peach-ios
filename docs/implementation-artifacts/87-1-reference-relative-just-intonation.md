...
- `SettingsCoordinator.playSoundPreview(note:duration:)` (74) — **not in original Code Map**; single-note Settings preview through `userSettings.tuningSystem`. Not a trial-playback path → initially left on the absolute bridge. *Resolved 2026-07-17 post-done (Michael): switched to ET — a lone preview tone has no interval to tune; now consistent with trial reference tones.*
...
- [x] **Task 3 — Tests first: TuningSystem reference-relative API.** Write failing tests for matrix rows 1-5, 7, 10. Implement `intervalCents(for:)` + `frequency(for:detunedBy:above:)`. (AC: rows 1-5, 7, 10 green; existing `TuningSystemTests` untouched and green.) *Final signature (after Michael's post-review API critique + Adam consult #2): `frequency(for: DetunedDirectedInterval, from: MIDINote, referencePitch: Frequency)` — new `DetunedDirectedInterval` domain type mirrors `DetunedMIDINote`; the reference passes as a logical `MIDINote` so every `frequency` overload has exactly one physical parameter, and the reference-note-is-ET rule lives inside the bridge. Row 5's "numerically identical (ulp tolerance)" is asserted as < 10⁻⁶ cents: the two paths differ in floating-point operation order (one `pow` vs. two), so bit-identity is not attainable; 10⁻⁶¢ is ~5 orders of magnitude below the 0.1¢ audio-precision requirement.*
...
- [x] **Task 7 — Gates + listening test.** *Listening test explicitly waived by Michael (2026-07-17): "My judgment isn't good enough for a JI listening test. We have to go by the code review." Gate replaced by a dedicated `/code-review high` sweep over the post-review delta (weighted toward audible-behavior correctness) + hand-computed frequency-oracle tests; all 5 findings fixed. Waiver recorded in `[[feedback_verify_audio_features]]`; it covers the pitch-accuracy class only.* All four schemes green (sequential). Michael listening test per Boundaries (JI M3 from C♯, ET control). (AC: gates green; Michael confirms; then status → review, and after review → done per `[[feedback_update_status_after_review]]`.) *Gates green 2026-07-17: iOS 2169 / macOS 2156 / iOS Research 2333 / macOS Research 2320; `archlint` clean; `check-dependencies.sh` sole finding is the pre-existing PF-070 comment-text false positive (fails identically on baseline). Michael listening test pending.*
...
**Adam consult #2 (2026-07-17) — API-shape renegotiation (Michael's post-review critique).** (a) The from-note is the **tuning anchor**, i.e. the codebase's existing "reference note" — NOT a scale root (a dyad has no scale) and NOT to be named `root` (collides with Hindemith interval-root theory, under which a descending fifth's root is the *target*). Parameter: `from referenceNote: MIDINote`; the `MIDINote`/`Frequency` type split disambiguates it from `referencePitch`. (b) `DetunedDirectedInterval` (directed interval + cent offset) is musically coherent — a mistuned interval — and mirrors `DetunedMIDINote`. Its offset is **pitch-space on the far tone** (positive = sharper target), not interval-width space; these differ by sign for descending intervals; must be documented. (c) Assumptions to state explicitly on the bridge: interval reckoned from the reference note per its direction; the reference note's own pitch is equal-tempered in every tuning system (imperceptible for a contextless tone; A-rooted alternative reintroduces the root lottery); detune applies to the far tone in pitch space. Passing the reference as `MIDINote` moves assumption (b) inside `TuningSystem` — single home.

**Adam consult #2 amendment (2026-07-17) — Michael's refutation accepted.** "A dyad has no scale" was too strong: the 5-limit ratio table is itself scale-derived (choosing 45/32 vs 64/45, 9/8 vs 10/9, 9/5 vs 16/9 are scale-on-a-root decisions), so the bridge *does* involve an implicit scale, and the design assumes **root = reference note, re-rooted every trial**. What a dyad lacks is only the need for mutual consistency among ≥3 sounding notes. The Hindemith `root`-name objection was made under the refuted framing and is withdrawn — inside `TuningSystem`, "root" reads unambiguously as scale root. Resolution: parameter renamed `from root: MIDINote`; call sites pass `trial.referenceNote`, keeping the root=reference conflation visible where it is decided; doc comment states the implicit-scale assumption. On the record: for a two-note trial the root choice is benign (either note as root yields the same target frequency — the ratio class is fixed by the directed interval); it becomes load-bearing only with a root persisting across notes (future drone/tonal-context discipline), which needs a root distinct from the sounding notes and the absolute A-rooted bridge.

**Correction (2026-07-17, final gate review):** the "same target frequency" phrasing above was wrong — only the *sounding interval* (cent relationship) is root-symmetric. Because the bridge pins the root to its ET pitch, re-rooting at the other note shifts the pair's absolute placement (and hence the target's frequency) by the interval's tuning deviation (~13.7¢ for a JI M3). The root must be the note the rest of the trial math anchors on; anchoring on one note while rooting the bridge on the other would silently collapse JI toward ET. Doc comment on `frequency(for:from:referencePitch:)` corrected accordingly.
...
- New API is two methods on `TuningSystem`: `intervalCents(for: DirectedInterval) -> Cents` (signed) and `frequency(for: DetunedDirectedInterval, from: MIDINote, referencePitch:)` (renegotiated shape, see Change Log 2026-07-17 API entry). One playback path for both tuning systems — no session-level branch anywhere.
...
**Tests:** `PeachTests/Core/Music/TuningSystemTests.swift`, `PeachTests/Core/Training/PitchDiscriminationTrialTests.swift`, `PeachTests/Training/PitchMatching/PitchMatchingSessionTests.swift`, `PeachTests/Training/PauseResumeContractTests.swift`; mechanical `interval:` constructor updates in: `PeachTests/Core/Algorithm/KazezNoteStrategyTests.swift`, `PeachTests/Core/Data/TrainingDataStoreTests.swift`, `PeachTests/Core/Profile/PerceptualProfileTests.swift`, `PeachTests/Core/Profile/PitchMatchingProfileTests.swift`, `PeachTests/Core/Profile/ProgressTimelineTests.swift`, `PeachTests/Core/Training/ObserverAdapterTests.swift`, `PeachTests/Profile/ProfileScreenTests.swift`, `PeachTests/Settings/SettingsTests.swift`, `PeachTests/Training/PitchDiscrimination/MockNextPitchDiscriminationStrategy.swift`, `PeachTests/Training/PitchDiscrimination/PitchDiscriminationTestHelpers.swift`, `PeachTests/Training/PitchDiscrimination/PitchDiscriminationSessionTests.swift`, `PeachTests/Training/PitchDiscrimination/PitchDiscriminationSessionDifficultyTests.swift`, `PeachTests/Training/PitchDiscrimination/PitchDiscriminationSessionIntegrationTests.swift`, `PeachTests/Training/PitchDiscrimination/PitchDiscriminationSessionLoudnessTests.swift`, `PeachTests/Training/PitchDiscrimination/PitchDiscriminationSessionResetTests.swift`, `PeachTests/Training/PitchDiscrimination/PitchDiscriminationSessionUserDefaultsTests.swift`, `PeachTests/Training/PitchDiscrimination/HapticFeedbackManagerTests.swift`
...
- 2026-07-17 — **Post-done follow-up (Michael):** Settings note preview switched from the user's tuning system (absolute A-rooted under JI) to equal temperament — a lone tone has no interval to tune; previews now match trial reference tones. TDD (red test pinned the 13.7¢ C♯ deviation first). Gates green: iOS 2209 / macOS 2196 / iOS Research 2373 / macOS Research 2360.
- 2026-07-17 — **Final gate: listening test waived, review substituted.** Michael waived the JI listening test (judgment below the ~14¢ threshold); a dedicated `/code-review high` sweep over the unreviewed delta became the gate. 5 findings, all fixed: (1) CONFIRMED false doc contract — "either note as root yields the same target frequency" corrected to sounding-interval-only symmetry (the ET-pinned root shifts absolute placement by the tuning deviation; wrong rooting would collapse JI toward ET) — also corrected in Consultation Findings; (2) PLAUSIBLE stale-ordering hazard among unstructured per-drag adjust Tasks — adjustPitch now re-derives the frequency from live slider state at task execution time and skips when no handle exists; (3) duplicated trial frequency methods — new `Core/Training/PitchTrial` protocol single-homes `referenceFrequency`/`inTuneTargetFrequency` for both trial types; (4) hand-rolled polling loops replaced with the existing `waitForCondition` helper; (5) root-relative/reference-relative terminology drift unified to root-relative (arc42 ×2, TuningSystem MARK + absolute-bridge doc). Gates green: iOS 2208 / macOS 2195 / iOS Research 2372 / macOS Research 2359; archlint clean. Story → done.
- 2026-07-17 — **Slider fix + matching symmetry (Michael's directive).** Slider: tunable playback now starts at the touched slider position (was: always the center) and the play-in-flight window is closed — a handle arriving after in-flight adjustments syncs to the latest slider value, and a handle arriving after commit/stop (state no longer `.playingTunable`) is stopped instead of orphaned into feedback. Symmetry: `PitchMatchingTrial` owns `referenceFrequency(referencePitch:)` / `inTuneTargetFrequency(tuningSystem:referencePitch:)` mirroring the discrimination trial (ET-reference rule now single-homed in trial + bridge); `CompletedPitchMatchingTrial` carries `interval` (coherence precondition; both matching adapters classify/persist via it — last note-arithmetic derivations gone); session's `inTuneTargetFrequency` became a computed property derived from the current trial (lockstep hazard eliminated by construction). Three new session tests (touch-position start, in-flight adjust sync, commit-in-flight stop) + new `PitchMatchingTrialTests`. Test-churn: 16 `CompletedPitchMatchingTrial` constructor sites gained `interval:`. Gates green: iOS 2208 / macOS 2195 / iOS Research 2372 / macOS Research 2359.
- 2026-07-17 — **API renegotiation on Michael's critique** (after the sweeps): the reference-relative bridge takes a new `DetunedDirectedInterval` (directed interval + pitch-space cent offset, mirroring `DetunedMIDINote`) and the reference note as a logical `MIDINote` — `frequency(for:from:referencePitch:)` — so `frequency` overloads carry exactly one physical parameter and the reference-note-is-ET rule is encoded once, inside the bridge. Adam consult #2 initially recommended `referenceNote` as the parameter name; Michael refuted the underlying "dyad has no scale" framing (the 5-limit table is scale-derived, so an implicit scale + root IS involved and the design assumes root = reference note). Adam conceded; final parameter is `from root: MIDINote` with the implicit-scale and root=reference assumptions stated in the doc comment, plus the on-record note that the root choice is benign for dyads and load-bearing only for a future persistent-root discipline. Offset semantics pinned as pitch-space-on-the-far-tone. Behavior unchanged throughout (ET-equivalence and all matrix tests pass unmodified in expectation). Fallout: `Direction` and `DirectedInterval` gained `nonisolated` (required for the nonisolated `DetunedDirectedInterval`'s synthesized conformances, matching `Interval`/`Cents`/`Frequency`).
- 2026-07-17 — Post-workflow `/code-review high` sweep (4 confirmed findings) + mandatory `/simplify-code` pass (4 parallel reviewers). Applied: `PitchDiscriminationStoreAdapter` uses `trial.interval.interval.semitones` instead of re-deriving from MIDI distance (the missed sibling of the step-04 adapter patch); `DirectedInterval.isUnison` unifies four spellings of the unison predicate (both adapters'/strategy's/sessions' sites); shared `centsAbove` test oracle replaces four copies; wolf-root/ET-equivalence tests parameterized with domain-typed arguments. Left alone deliberately: `Frequency`-shift helper extraction (the `pow` expression is the definition of cents; two in-file copies don't justify indirection), trial-init precondition dedup (two one-liners), `inTuneTargetFrequency` as computed property + `CompletedPitchMatchingTrial.interval` field (both fold into the pending trial-API design decision for Michael). Gates re-run green: iOS 2196 / macOS 2183 / iOS Research 2360 / macOS Research 2347.
...
- The new bridge: pure ratio from the reference note, detune on top; one path for both tuning systems
  [`TuningSystem.swift:87`](../../Peach/Core/Music/TuningSystem.swift#L87)

- New domain type: a directed interval detuned in pitch space, mirroring `DetunedMIDINote`
  [`DetunedDirectedInterval.swift:12`](../../Peach/Core/Music/DetunedDirectedInterval.swift#L12)
