...
**Always:**
- On surfaces shared by more than one discipline — the Start cards and the profile charts — the unit rendered comes from the discipline's config. No *shared* view hardcodes a unit. Discipline-specific views may hold their unit literally, since only one discipline ever renders them; `PitchMatchingFeedbackIndicator`, `RhythmTimingFeedbackIndicator`, `TimingStatsView`, `ChromaticTrialResultView` and `TrainingStatsView` all do, all correctly, and this story does not change them. *(Narrowed 2026-08-08 on Michael's instruction — the original unconditional wording was false as written; see Spec Change Log.)*
