...
- [ ] Task 1: Add Mac destination in Xcode (AC: #1)
  - [ ] 1.1 In Xcode project editor > General > Supported Destinations, add "Mac"
  - [ ] 1.2 Xcode will prompt about unsupported frameworks — accept and review changes
  - [ ] 1.3 Set macOS deployment target to macOS 26.0 (matching iOS 26.0 strategy)

- [ ] Task 2: Fix compilation errors (AC: #2, #3)
  - [ ] 2.1 Build for macOS, collect all errors
  - [ ] 2.2 In `SoundFontEngine.swift`: wrap `configureAudioSession()` body in `#if os(iOS)` with empty `#else` (details in story 66.2)
  - [ ] 2.3 In `AudioSessionInterruptionMonitor.swift`: wrap `AVAudioSession` notification observers in `#if os(iOS)` (details in story 66.2)
  - [ ] 2.4 In `HapticFeedbackManager.swift`: wrap entire file or `import UIKit` + generator in `#if os(iOS)` (details in story 66.4)
  - [ ] 2.5 In `PeachApp.swift`: conditionalise `UIApplication` notification name references and `HapticFeedbackManager` instantiation (details in stories 66.3, 66.4)
  - [ ] 2.6 In test files using `UIKit` (`PitchMatchingSessionTests`, `AudioSessionInterruptionMonitorTests`): add `#if os(iOS)` guards around iOS-specific test code
  - [ ] 2.7 Fix any other compilation errors that surface

- [ ] Task 3: Verify SPM dependencies (AC: #4)
  - [ ] 3.1 Confirm MIDIKit resolves for macOS target
  - [ ] 3.2 Confirm swift-timecode resolves for macOS target

- [ ] Task 4: Verify iOS tests still pass (AC: #5)
  - [ ] 4.1 Run full test suite on iOS Simulator
