...
- [ ] Task 1: Create `App/Platform/` directory and `BackgroundPolicy` abstraction (AC: #1, #8)
  - [ ] 1.1 Create `Core/Ports/BackgroundPolicy.swift` protocol
  - [ ] 1.2 Create `App/Platform/IOSBackgroundPolicy.swift` (unconditional)
  - [ ] 1.3 Create `App/Platform/MacOSBackgroundPolicy.swift` (unconditional)
  - [ ] 1.4 Add `backgroundPolicy` param to `TrainingLifecycleCoordinator.init`, remove `#if os`
  - [ ] 1.5 Wire in `PeachApp.swift` with `#if os` selection at composition root
  - [ ] 1.6 Update `TrainingLifecycleCoordinatorTests` -- remove `#if os` blocks, inject specific policies
  - [ ] 1.7 Add `BackgroundPolicyTests.swift`

- [ ] Task 2: Create `AudioSessionConfiguring` abstraction (AC: #2)
  - [ ] 2.1 Create `Core/Ports/AudioSessionConfiguring.swift` protocol
  - [ ] 2.2 Create `App/Platform/IOSAudioSessionConfigurator.swift` (`#if os(iOS)` file-level)
  - [ ] 2.3 Create `App/Platform/MacOSAudioSessionConfigurator.swift` (`#if os(macOS)` file-level)
  - [ ] 2.4 Add `audioSessionConfigurator` param to `SoundFontEngine.init`, replace `Self.configureAudioSession()` calls
  - [ ] 2.5 Wire in `PeachApp.swift`

- [ ] Task 3: Create `AudioInterruptionObserving` abstraction (AC: #3)
  - [ ] 3.1 Create `Core/Ports/AudioInterruptionObserving.swift` protocol
  - [ ] 3.2 Create `App/Platform/IOSAudioInterruptionObserver.swift` (`#if os(iOS)` file-level) -- move interruption + route change logic from AudioSessionInterruptionMonitor
  - [ ] 3.3 Create `App/Platform/NoOpAudioInterruptionObserver.swift` (no guard)
  - [ ] 3.4 Refactor `AudioSessionInterruptionMonitor` to accept and delegate to `AudioInterruptionObserving`
  - [ ] 3.5 Wire in `PeachApp.swift`
  - [ ] 3.6 Update `AudioSessionInterruptionMonitorTests` -- use mock observer

- [ ] Task 4: Centralize platform notification names (AC: #4)
  - [ ] 4.1 Create `App/Platform/PlatformNotifications.swift`
  - [ ] 4.2 Update `PeachApp.swift` to use `PlatformNotifications.background` / `.foreground`

- [ ] Task 5: Centralize platform image encoding (AC: #5)
  - [ ] 5.1 Create `App/Platform/PlatformImage.swift`
  - [ ] 5.2 Update `ChartImageRenderer.pngData` to delegate to `PlatformImage`

- [ ] Task 6: Split HapticFeedbackManager (AC: #6)
  - [ ] 6.1 Create `App/Platform/HapticFeedbackManager.swift` with iOS implementation (`#if os(iOS)`)
  - [ ] 6.2 Create `App/Platform/NoOpHapticFeedbackManager.swift` (no guard)
  - [ ] 6.3 Delete `PitchDiscrimination/HapticFeedbackManager.swift`
  - [ ] 6.4 Update `HapticFeedbackManagerTests` imports

- [ ] Task 7: Verify and run tests (AC: #7, #9)
  - [ ] 7.1 `bin/check-dependencies.sh` -- Core/ stays framework-free
  - [ ] 7.2 `bin/test.sh && bin/test.sh -p mac` -- all tests pass
