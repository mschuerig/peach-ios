...
### PF-088: Pitch profile headlines render without a unit

**Found:** 2026-08-08 (code review of story 83.5; scope corrected twice the same day after checking the running app)
**Severity:** Medium
**Disposition:** OPEN

`Peach/Profile/ProgressChartView.swift:72-77` renders `Text(ChartData.formatEWMA(ewma))` and `Text(ChartData.formatStdDev(stddev))` with **no unit**; `Peach/Profile/ExportChartView.swift:52-54` likewise. The unit appears only on the Y-axis label, visually distant from the headline figure. The four pitch disciplines use this card, so their headlines read `11.5` and `±0.0`.

The rhythm half of this entry has been **fixed**: `RhythmProfileCardView` now takes its unit from `config.unitSymbol` and formats through `MetricValueFormatter`, so the Profile screen reads `79.5 ms` / `±7.1 ms` from configuration rather than literals.

Two earlier drafts of this entry were wrong and are corrected here: the first claimed the *timing* headline lacked a unit (it is the pitch headlines that do), and the second claimed `RhythmProfileCardView` was discipline-specific and therefore exempt from story 83.5's narrowed constraint — it is rendered by **both** rhythm disciplines (`TimingOffsetDetectionDiscipline.swift:43`, `ContinuousRhythmMatchingDiscipline.swift:38`) and so was never exempt.

**Fix:** Give the pitch headline its unit from `config.unitSymbol`, matching the rhythm card's now-corrected treatment. Both views already hold `config`.
