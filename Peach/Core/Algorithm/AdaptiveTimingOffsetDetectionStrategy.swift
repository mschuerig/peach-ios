...
final class AdaptiveTimingOffsetDetectionStrategy: NextTimingOffsetDetectionStrategy {
    ...
    private let logger = Logger(subsystem: "com.peach.app", category: "AdaptiveTimingOffsetDetectionStrategy")
    ...
    func nextTimingOffsetDetectionTrial(
        ...
        let offset = TimingOffset(signedDuration)
        ...
        return TimingOffsetDetectionTrial(tempo: settings.tempo, offset: offset)
    }

    // MARK: - Direction Selection

    private func chooseDirection(profile: TrainingProfile, settings: TimingOffsetDetectionSettings) -> TimingDirection {
        guard let tempoRange = TempoRange.range(for: settings.tempo) else {
            return Bool.random() ? .early : .late
        }

        let earlyStats = profile.statistics(for: .rhythm(.timingOffsetDetection, tempoRange, .early))
        let lateStats = profile.statistics(for: .rhythm(.timingOffsetDetection, tempoRange, .late))
    ...
    private func choosePercentage(
        ...
        if case .continuous(let stats) = profile.statistics(for: .rhythm(.timingOffsetDetection, tempoRange, direction)) {
