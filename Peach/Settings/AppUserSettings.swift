...
final class AppUserSettings: UserSettings {
    ...
    var noteRange: NoteRange {
        guard let lower = defaults.object(forKey: SettingsKeys.noteRangeMin) as? Int,
              let upper = defaults.object(forKey: SettingsKeys.noteRangeMax) as? Int,
    ...
    var noteDuration: NoteDuration {
        guard let raw = defaults.object(forKey: SettingsKeys.noteDuration) as? Double else {
    ...
    var referencePitch: Frequency {
        guard let raw = defaults.object(forKey: SettingsKeys.referencePitch) as? Double else {
    ...
    var soundSource: any SoundSourceID {
        let raw = defaults.string(forKey: SettingsKeys.soundSource) ?? SettingsKeys.defaultSoundSource
        return SoundSourceTag(rawValue: raw)
    }

    var varyLoudness: UnitInterval {
        guard let raw = defaults.object(forKey: SettingsKeys.varyLoudness) as? Double else {
    ...
    var tempoBPM: TempoBPM {
        let raw = defaults.integer(forKey: SettingsKeys.tempoBPM)
    ...
    var tuningSystem: TuningSystem {
        guard let raw = defaults.string(forKey: SettingsKeys.tuningSystem),
